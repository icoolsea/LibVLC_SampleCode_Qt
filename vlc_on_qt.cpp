...
Player::Player() : QWidget() {
  ...
  _vlcinstance =
      libvlc_new(sizeof(vlc_args) / sizeof(vlc_args[0]),
  ...
  // connect the two sliders to the corresponding slots (uses Qt's signal /
  // slots technology)
  ...
  poller->start(
      100); // start timer to trigger every 100 ms the updateInterface slot
}
...
void Player::playFile(QString file) {
  // the file has to be in one of the following formats /perhaps a little bit
  // outdated)
  /*
  ...
  */
  ...
  //_m = libvlc_media_new_location(_vlcinstance,
...
/* Get our media instance to use our window */
  ...
  libvlc_media_player_set_drawable(
      _mp, reinterpret_cast<unsigned int>(_videoWidget->winId()), &_vlcexcep);
 // libvlc_media_player_set_hwnd(_mp, _videoWidget->winId(), &_vlcexcep ); // for
 // vlc 1.0
#elif defined(Q_OS_MAC)
  libvlc_media_player_set_drawable(_mp, _videoWidget->winId(), &_vlcexcep);
 // libvlc_media_player_set_agl (_mp, _videoWidget->winId(), &_vlcexcep); // for
 // vlc 1.0
#else // Linux
  //[20101201 Ondrej Spilka] obsolete call on libVLC >=1.1.5
  // libvlc_media_player_set_drawable(_mp, _videoWidget->winId(), &_vlcexcep );
  // libvlc_media_player_set_xwindow(_mp, _videoWidget->winId(), &_vlcexcep );
  // // for vlc 1.0
